...
Theorem remove_after_update : forall X x v (m : partial_map X),
  remove (m & {{ x --> v }}) x = m.
Proof.
  intros X x v m. unfold update. unfold remove.
  rewrite t_update_shadow.
  unfold t_update.
Admitted.

Inductive same_within {X} : partial_map X -> partial_map X -> list string -> Prop :=
| SW_nil : forall (m m': partial_map X), (forall s, m s = m' s) -> same_within  m m' []
| SW_cons : forall (m m': partial_map X) h t,
    (exists v, m h = Some v /\ m' h = Some v) ->
    same_within m m' t -> same_within m m' (h :: t).

Inductive same_besides {X} : partial_map X -> partial_map X -> list string -> Prop :=
| SB_nil : forall (m m': partial_map X), (forall s, m s = m' s) -> same_besides m m' []
| SB_cons : forall (m m': partial_map X) h t, m h = m' h -> same_besides (remove m h) (remove m' h) t -> same_besides m m' (h :: t).
